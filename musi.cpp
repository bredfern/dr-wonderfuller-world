...
extern signed short int *instruments;
extern long numInstruments;
extern long instrumentLength;



extern signed short int *drums;
extern long numDrums;
extern long drumLength;

int bufferIndex = 0;
signed short int buffer[180 * 44100];
...
class Instrument {
	...
	Instrument(signed short int *xs,int n,int l,int ch,int srate) {
		wave = &(xs[(rand() % n) * l]);
		this->l = l;
		nch = ch;
		sr = srate;
	...
	virtual int render(int index,int scaleIndex,double fr,float vol);
};
int Instrument::render(int index,int scaleIndex,double fr,float vol) {
	int n = rand() % 500;
	...
	for(int i = 0;i < 20000;++i) 
		if(i + index + n < sizeof(buffer) / sizeof(buffer[0]))
			...
			for(int k = 0;k < nch;++k)
			{
				buffer[i + index + n] += (int)(0.1 * vol * (double)wave[(int)(i * fr) * nch + k]);
			}
	
	return (int)(20000.0 * fr);
}

int mySynthCallback(short *buf, int num, espeak_EVENT *evt);

int singerSampleRate;
...
class Singer : public Instrument {
	...
	Singer() : Instrument(instruments,numInstruments,instrumentLength,1,singerSampleRate) {
		espeak_SetSynthCallback(mySynthCallback);
	
		l = 0;
		done = false;
		id = 0;
		espeak_SetVoiceByName("mb-de7");
		const char *s = "la a a a le e e lu u u nur der Mann im Mond schaut zuuuu, wenn die kleinen Babies schlaaaaafen, la a a a le e e lu u u u";
		espeak_Synth(s,strlen(s),0,	POS_CHARACTER,0,espeakCHARS_8BIT,&id,this);
		
		while(!done) usleep(100);		
	}
	
	int render(int index,int key,double fr,float vol) override;
};
int Singer::render(int index,int key,double fr,float vol) {
	key = key % 12;
	
	int n = Instrument::render(index,key,fr,vol * 3.8);
	if(n > 5500) n = 5500;
	
	wave = &wave[n];
	if(wave > buf + l - 10) wave = buf;
	return 0;
}
...
int mySynthCallback(short *buf, int num, espeak_EVENT *evt) {
	...
	for(int i = 0;i < num;++i) s->buf[i + s->l] = buf[i];
...
struct Drum {
	...
	Drum() : ins(drums,numDrums,drumLength,2,44100) {
	}
	
	void render(int index) {
		if(rand() % 100 < probDoDrum)
			ins.render(index,-1,1.0,1.5);
	}
};
...
struct Melody {
	...
	void init() {
		...
		fractal(0,sizeof(tones) / sizeof(tones[0]) - 1,0.9,sizeof(scale) / sizeof(scale[0]) * (rand() % 2 ? 1.0 : -1.0));
		for(int i = 0;i < sizeof(tones) / sizeof(tones[0]);++i) printf("%f,",tones[i]);	
		printf("\n");	
	...
	Melody() : ins(new Instrument(instruments,numInstruments,instrumentLength,2,44100)) {
		init();
	}
...
int main(int argc, char **argv) {
	...
	for(int i = 0;i < numInstruments;++i) {
		complex<double> xs[1 << 16];
		int n = sizeof(xs) / sizeof(xs[0]);
		
		short *wave = &instruments[i * instrumentLength];
		
		for(int j = 0;j < n;++j) xs[j] = wave[j * 2];// + wave[j * 2 + 1];
		...
		double avg = 0;
		for(int j = 0;j < n;++j) avg += abs(xs[j]);
		avg /= n;
		
		int j0 = 0;
		int j1 = 0;
		for(int j = 0;j < n;++j) if(abs(abs(xs[j]) - avg) < abs(abs(xs[j0]) - avg)) j0 = j;
		for(int j = 0;j < n;++j) if(abs(abs(xs[j])) > abs(abs(xs[j1]))) j1 = j;
		cout << "++++++ " << j0 << endl;
		...
		int j;
		for(j = 0;j < n && j < n - j0;++j) xs[j] = xs[j + j0];
		for(;j < n;++j) xs[j] = 0;
		
		j0 = 0;
		for(int j = 0;j < n;++j) if(abs(xs[j]) > abs(xs[j0])) j0 = j;
		//complex<double> x = xs[j0];
		//for(int j = 0;j < n;++j) xs[j] /= abs(x);
		//for(int j = 0;j < n;++j) xs[j] *= -atan((float)(j + j0)/ (float)n - M_PI_2) + M_PI_2;
		...
		double max = -1E20;
		for(int j = 0;j < n;++j) {
			if(xs[j].real() > max) max = xs[j].real();
			if(xs[j].real() < min) min = xs[j].real();
		}
		
		for(int j = 0;j < n;++j)
			xs[j] /= abs(min) > abs(max) ? abs(min) : abs(max);
		for(int j = 0;j < n;++j) xs[j] *= 30000.0;
		
		for(int j = 0;j < n;++j) wave[2 * j] = (int)xs[j].real();
		for(int j = 0;j < n;++j) wave[2 * j + 1] = (int)xs[j].real();
		for(int j = n * 2;j < instrumentLength;++j) wave[j] = 0;
		
		for(int j = 0;j < 20;++j) printf("%d , ",wave[j]);
		printf("\n");
	}*/
	
	for(int i = 0;i < numDrums;++i) {
		short *wave = &drums[i * drumLength];
		
		long min = 100000000;
		long max = -100000000;
		
		for(int j = 0;j < drumLength;++j) if(wave[j] < min) min = wave[j];
		for(int j = 0;j < drumLength;++j) if(wave[j] > max) max = wave[j];

		for(int j = 0;j < drumLength;++j)
			wave[j] = (long)wave[j] * 20000.0 / (abs(min) > abs(max) ? abs(min) : abs(max));
	}	
	for(int i = 0;i < numInstruments;++i) {
		short *wave = &instruments[i * instrumentLength];
		...
		for(int j = 0;j < instrumentLength;++j) if(wave[j] < min) min = wave[j];
		for(int j = 0;j < instrumentLength;++j) if(wave[j] > max) max = wave[j];

		for(int j = 0;j < instrumentLength;++j)
			wave[j] = (long)wave[j] * 20000.0 / (abs(min) > abs(max) ? abs(min) : abs(max));
	}	
	
	singerSampleRate = espeak_Initialize(AUDIO_OUTPUT_RETRIEVAL, 120 * 44100, 0, 0);
	printf("singerSampleRate %d\n",singerSampleRate);
